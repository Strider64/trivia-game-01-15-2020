...
class Trivia {
    ...
    public function read($category) {
        $db = DB::getInstance();
        $pdo = $db->getConnection();
        $this->query = "SELECT id, question, answer1, answer2, answer3, answer4, correct, category FROM trivia_questions WHERE category=:category";
        $this->stmt = $pdo->prepare($this->query);
    ...
    public function check(array $data) {
        ...
        $this->stmt->execute([':id' => $data['id']]);

        $this->data = $this->stmt->fetchALL(PDO::FETCH_ASSOC);    
        
        return array_merge($this->data[0], $data);
    }
